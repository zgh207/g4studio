...
/**
 * This class implements a bidirectional iterator over an FTPFileList. Elements
 * may be retrieved one at at time using the hasNext() - next() syntax familiar
 * from Java 2 collections. Alternatively, entries may be receieved as an array
 * of any requested number of entries or all of them.
 * 
 ...
 *             version 2.0 - use FTPFileParseEngine instead
 */
public class FTPFileIterator {
	...
	/**
	 * constant shorthand for the situation where the raw listing has not yet
	 * been scanned
	 */
	...
	/**
	 * "Package-private" constructor. Only the FTPFileList can create an
	 * iterator, using it's iterator() method. The list will be iterated with
	 * the list's default parser.
	 * 
	 * @param rawlist
	...
	/**
	 * "Package-private" constructor. Only the FTPFileList can create an
	 * iterator, using it's iterator() method. The list will be iterated with a
	 * supplied parser
	 * 
	 * @param rawlist
	 *            the FTPFileList to be iterated
	 ...
	 * 
	 * @param entry
	 *            A string containing one entry, as determined by the parser's
	 *            getNextEntry() method.
	 * 
	...
	/**
	 * Skips over any introductory lines and stuff in the listing that does not
	 * represent files, returning the line number of the first entry that does
	 * represent a file.
	 * 
	 ...
	 * elements are available, the returned array will have a length equal to
	 * the number of entries at and after after the current position. If no such
	 * entries are found, this array will have a length of 0.
	 * 
	 * After this method is called the current position is advanced by either
	 * <code>quantityRequested</code> or the number of entries available after
	 * the iterator, whichever is fewer.
	 * 
	 * @param quantityRequested
	 *            the maximum number of entries we want to get. A 0 passed here
	 *            is a signal to get ALL the entries.
	 * 
	 ...
	 * non-null value.
	 * 
	 * @return true if there exist any files after the one currently pointed to
	 *         by the internal iterator, false otherwise.
	 */
	 ...
	 * 
	 * @return a single FTPFile object corresponding to the raw input line at
	 *         the position of the internal iterator over the list of raw input
	 ...
	 * <code>quantityRequested</code> such elements are available, the returned
	 * array will have a length equal to the number of entries after the
	 * iterator. If no such entries are found, this array will have a length of
	 * 0. The entries will be ordered in the same order as the list, not
	 * reversed.
	 * 
	 * After this method is called the current position is moved back by either
	 * <code>quantityRequested</code> or the number of entries available before
	 * the current position, whichever is fewer.
	 * 
	 * @param quantityRequested
	 *            the maximum number of entries we want to get. A 0 passed here
	 *            is a signal to get ALL the entries.
	 * @return an array of at most <code>quantityRequested</code> FTPFile
	 *         objects starting at the position preceding the current position
	 *         of this iterator within its list and at least the number of
	 *         elements which exist in the list prior to its current position.
	 */
	 ...
	 * non-null value.
	 * 
	 * @return true if there exist any files before the one currently pointed to
	 *         by the internal iterator, false otherwise.
	 */
	...
	/**
	 * Returns a single parsed FTPFile object corresponding to the raw input
	 * line at the position preceding that of the internal iterator over the
	 * list of raw lines maintained by this object
	 * 
	 * After this method is called the internal iterator is retreated by one
	 * element (unless it is already at beginning of list).
	 * 
	 * @return a single FTPFile object corresponding to the raw input line at
	 *         the position immediately preceding that of the internal iterator
