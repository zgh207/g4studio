 ...
 * lines, each of which (after some possible initial verbiage) represents a file
 * on the FTP server. A parser is also supplied, which is used to iterate
 * through the internal list of lines parsing each into an FTPFile object which
 * is returned to the caller of the iteration methods. This parser may be
 * replaced with another, allowing the same list to be parsed with different
 * parsers. Parsing takes place on an as-needed basis, basically, the first time
 * a position is iterated over. This happens at the time of iteration, not prior
 * to it as the older <code>(FTPClient.listFiles()</code> methods did, which
 * required a bigger memory hit.
 * 
...
public class FTPFileList {
	...
	/**
	 * The only constructor for FTPFileList, private because construction only
	 * invoked at create()
	 * 
	 * @param parser
	 *            a <code>FTPFileEntryParser</code> value that knows how to
	 *            parse the entries returned by a particular FTP site.
	 * @param encoding
	 *            The encoding to use.
	 */
	...
	/**
	 * The only way to create an <code>FTPFileList</code> object. Invokes the
	 * private constructor and then reads the stream supplied stream to build
	 * the intermediate array of "lines" which will later be parsed into
	 * <code>FTPFile</code> object.
	 * 
	 * @param stream
	 *            The input stream created by reading the socket on which the
	 *            output of the LIST command was returned
	 * @param parser
	 *            the default <code>FTPFileEntryParser</code> to be used by this
	 *            object. This may later be changed using the init() method.
	 * @param encoding
	 *            The encoding to use
	 * 
	 * @return the <code>FTPFileList</code> created, with an initialized of
	 *         unparsed lines of output. Will be null if the listing cannot be
